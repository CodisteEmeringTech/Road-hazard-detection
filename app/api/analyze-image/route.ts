...
export async function POST(request: NextRequest) {
  try {
    ...
    const description = formData.get("description") as string; // Single input: user description containing everything
    ...
    // Build context-aware prompt with only description
    let contextualInfo = "";
    if (description) {
      contextualInfo += `\nUser description: ${description}`;
    }

    // Enhanced comprehensive road inspection prompt (your original prompt preserved)
...
Return JSON with:
{
  ...
  "locations": ["list of location entities detected, else empty"],
  "persons": ["list of person names detected, else empty"],
...
Respond ONLY with the JSON object.`
    ...
    const response = await openai.chat.completions.create({
      ...
      max_tokens: 1500, // for comprehensive analysis
      temperature: 0.3, // stable analysis
    });
    ...
    if (error?.error?.type) {
      ...
      switch (error.error.type) {
        ...
        case "permission_error":
          return NextResponse.json(
            { error: "Permission denied for AI service" },
            { status: 403 }
          );
        ...
        case "overloaded_error":
          return NextResponse.json(
            { error: "AI service is overloaded. Please try again" },
            { status: 503 }
          );
        default:
          return NextResponse.json(
